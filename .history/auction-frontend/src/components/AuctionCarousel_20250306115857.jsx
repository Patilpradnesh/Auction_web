...
const auctionItems = [
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 relative">
      ...
      <button className="absolute top-1/2 left-0 transform -translate-y-1/2 bg-black bg-opacity-50 text-white p-2 rounded-full" onClick={() => this.slider.slickPrev()}>
        &#9664;
      </button>
      <button className="absolute top-1/2 right-0 transform -translate-y-1/2 bg-black bg-opacity-50 text-white p-2 rounded-full" onClick={() => this.slider.slickNext()}>
        &#9654;
      </button>
    </div>
  );
