...
const Dashboard = () => {
  ...
  // Fetch active bids from the backend
  useEffect(() => {
    const fetchActiveBids = async () => {
      try {
        const response = await fetch("/api/active-bids"); // Replace with your backend API endpoint
  ...
  return (
    <div className="dashboard">
      ...
      <div className="active-bids-container">
        {activeBids.map((bid) => (
          <ActiveBidCard key={bid._id} bid={bid} />
        ))}
      </div>
    </div>
  );
...
//         <div className="bg-white rounded-lg shadow  border border-black mb-5">
