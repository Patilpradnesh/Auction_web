...
const AuctionCarousel = () => {
  ...
  const adjustImageSize = (url) => {
    const img = new Image();
    img.src = url;
    img.onload = () => {
      const aspectRatio = img.width / img.height;
      const carouselHeight = 400; // Set your desired carousel height
      const carouselWidth = carouselHeight * aspectRatio;
  ...
  auctionItems.forEach(item => {
    item.image = adjustImageSize(item.url);
  });
  const auctionItems = [
    {
      id: 1,
